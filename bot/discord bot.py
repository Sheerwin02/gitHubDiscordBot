...
# Dictionary to store the scheduled repositories and their tracked channels
...
async def send_commit_info(repository, channel_ids):
  ...
  if commit_info is not None:
    ...
    if repository in scheduled_repositories and last_commit_hashes.get(
...
async def on_message(message):
  ...
  # Check if the message content starts with the cancel_track command
  if message.content.startswith('!cancel_track'):
    # Parse the command arguments
    args = message.content.split()[1:]
    if len(args) != 2:
      await message.channel.send(
        'Invalid number of arguments. Usage: !cancel_track <repository> <channel>'
      )
      return
    repository, channel_name = args

    # Check if the repository is being tracked
    if repository in scheduled_repositories:
      channel_ids = scheduled_repositories[repository]
      # Get the channel to cancel the track
      channel = discord.utils.get(message.guild.channels, name=channel_name)
      if channel is None:
        await message.channel.send(f'Channel {channel_name} not found')
        return

      # Check if the channel is tracked for the repository
      if channel.id in channel_ids:
        # Remove the channel ID from the list of tracked channels
        channel_ids.remove(channel.id)
        # Save the scheduled repositories to file
        await save_scheduled_repositories()
        await message.channel.send(
          f'Successfully canceled commit information for {repository} '
          f'to be sent to {channel_name}',
          file=discord.File('cancel.gif'))
      else:
        await message.channel.send(
          f'The repository {repository} is not being tracked in {channel_name}'
        )
    else:
      await message.channel.send(
        f'The repository {repository} is not being tracked')
